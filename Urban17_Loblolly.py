# -*- coding: utf-8 -*-
"""
Code to fit PGEN to data for Loblolly pine from Urban et al. (2017)
...
popt, pcov = curve_fit( f, gt_co2_mol_all, A_all, p0 = [2.06847911e+01, 1.37935891e+01, 3.72933730e+01, 9.72280027e-04, 2.02649876e-04, 6.24972076e-03, 1.50000232e+00],
                        bounds = ([1,1,1,1e-15,1e-7, 0.0, 1.0],[200,100,50,1,1,0.1,5.0]))    
...
A_an_gs    = calc_A_from_gs_C3( pc, nl, Tl_all, gt_co2_mol_all, Ca_all, Pa_all, 
                                PAR_all, Oa_all, Ea * 1.0e3, eta, Topt, vcmax25, 
                                jmax25, fd, q10, rd_func = None ) * 1.0e6
# plt.figure()
# plt.scatter(A_an_gs, A_all, c = Tl_all, cmap = 'coolwarm' ) 
...
for i, df_main in enumerate( [ df_wet, df_dry, df_wet_co2 ] ):
    ...
    Oa    = Oa_pc * Pa / 100.0
    PAR   = 1200.0e-6
    PAR   = (df['PARi'] - df['PARo']) * 1e-6
    Tree_num = df.get('Tree No.')
    VPD_leaf = df['VpdL'] * 1e3
    VPD_air = df['VpdA'] * 1e3
    gs_co2_mol = gs_h2o_mol / 1.6
    gt_co2_mol = df['CndCO2']
    gs_h2o_m_s = gs_h2o_mol * 8.314462 * ( Ta + 273.15 ) / Pa
    ...
    A_an_gs    = calc_A_from_gs_C3( pc, nl, Tl, gt_co2_mol, Ca, Pa, PAR, Oa, Ea * 1.0e3, 
                                    eta, Topt, vcmax25, jmax25, fd, q10, 
                                    rd_func = None ) * 1.0e6
    vmin = Tl_all.min()
    vmax = Tl_all.max()
    axs[0,i].scatter(A_an_gs, A, c = Tl, cmap = 'coolwarm', vmin = vmin, vmax = vmax ) 
    ...
    # Non-linear least squares
    idx = Tl <= 45
    f_gcrit_LTO = lambda Tair, beta: numerical_solve_LTO( nl, pc, Ta[idx], Ca[idx], Pa[idx], Oa[idx], Is, ra, vpd = VPD_leaf[idx], 
                                                          Ipar = PAR[idx], rb_co2_mol = rb_co2_mol[idx], vpd_air = VPD_air[idx], beta = beta )[1]
    ...
    if i == 1:
        ...
        rp_p0_LTO = 0.0001
    popt, pcov = curve_fit( f_gcrit_LTO, Ta[idx], gs_co2_mol[idx], p0 = rp_p0_LTO, bounds = [0,0.4], diff_step = 1.0e-5 )
    print(popt)
    beta_LTO = popt[0]
    beta_LTO_SE = np.sqrt( np.diag( pcov ) )
    
    popt, pcov = curve_fit( f_gcrit_LT, Ta[idx].values, gs_co2_mol[idx].values, p0 = 0.0001, bounds = [0,0.4], diff_step = 1.0e-5 )
    print(popt)
    beta_LT = popt[0]
    ...
    save_params( params_path, 'Urban17_loblolly_%s_params.csv'%(save_labels[i]),
    ...
    save_out_data( out_data_path, 'Urban17_loblolly_%s_out.csv'%(save_labels[i]),
