...
fig.savefig('Figures/Figure_2.jpg', dpi = 300, bbox_inches = 'tight')
# A vs gs
# axs[3].plot( A_LTO, gs_LTO, color = line_colors['LTO'],
#                ls = line_style, marker = 'o', markerfacecolor = 'none' )
# axs[3].plot( A_LT, gs_LT, color = line_colors['LT'],
#               ls = line_style, marker = 'o', markerfacecolor = 'none' )
# axs[3].plot( A_AT, gs_AT, color = line_colors['AT'],
#                ls = line_style, marker = 'o', markerfacecolor = 'none' )
# for i in range(N):
#     axs[3].plot( A_LTO[i], gs_LTO[i], color = line_colors['LTO'],
#                  ls = line_style, marker = 'o', alpha = Ta[i] / Ta.max())
#     axs[3].plot( A_LT[i], gs_LT[i], color = line_colors['LT'],
#                  ls = line_style, marker = 'o', alpha = Ta[i] / Ta.max())
#     axs[3].plot( A_AT[i], gs_AT[i], color = line_colors['AT'],
#                  ls = line_style, marker = 'o', alpha = Ta[i] / Ta.max())
    
# im = axs[3].scatter( A_LTO, gs_LTO, c = Ta, marker = 's', cmap ='coolwarm', edgecolors = 'black', zorder = 0 )
# axs[3].scatter( A_LT, gs_LT, c = Ta, marker = 'o', cmap ='coolwarm', edgecolors = 'black', zorder = 0 )
# axs[3].scatter( A_AT, gs_AT, c = Ta, marker = '^', cmap ='coolwarm', edgecolors = 'black', zorder = 0 )

# cbar_ax = fig.add_axes([0.91, 0.14, 0.01, 0.3])
# cb = plt.colorbar(im, cax = cbar_ax )         
# cb.set_label('Atmospheric temperature ($\degree C$)', size = 15)

# custom_lines  = [Line2D([], [], color = 'black', ls = 'none', marker = 's' , markerfacecolor = 'none'),
#                  Line2D([], [], color = 'black', ls = 'none', marker = 'o' , markerfacecolor = 'none'),
#                  Line2D([], [], color = 'black', ls = 'none', marker = '^' , markerfacecolor = 'none'),
#                  ]
# axs[3].legend(custom_lines,[line_labels[s] for s in plot_schemes],loc = 'lower right')
           
# axs[3].set_ylabel('$g_{sc}$ (mol/m$^2$/s)',size = 15)
# axs[3].set_xlabel( '$A$ ($\mu$mol/m$^2$/s)', size = 15 )
# axs[3].text( 0.01, 0.98, '(d)', transform = axs[3].transAxes, 
#               ha = 'left', va = 'top', size = 12 )
...
im = axs[0].scatter( A_LTO, gs_LTO, c = Ta, marker = 'o', cmap ='coolwarm', edgecolors = 'black', zorder = 0 )
axs[1].scatter( A_LT, gs_LT, c = Ta, marker = 'o', cmap ='coolwarm', edgecolors = 'black', zorder = 0 )
axs[2].scatter( A_AT, gs_AT, c = Ta, marker = 'o', cmap ='coolwarm', edgecolors = 'black', zorder = 0 )
