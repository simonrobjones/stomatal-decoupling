...
from Figure_5_functions import numerical_solve_Diao_LTO, numerical_solve_Diao_LT, numerical_solve_Diao_AT
from Figure_5_functions import calc_rd_Diao, calc_J_Diao
from Figure_5_functions import bootstrap_loess
...
# Initialise figure
fig_5, axs_5 = plt.subplots( ncols = 3, nrows = 4, figsize = ( 18, 16 ) )
plt.subplots_adjust( wspace = 0.25, hspace = 0.25 )

# Add legend
fig_5.legend(custom_lines_legend,legend_labels,loc = 'upper center',bbox_to_anchor = (0.5,0.05), fontsize = 12, ncol = 5 )

# Label axes
for i, row in enumerate(axs_5):
    for j, ax in enumerate(row):
        ax.text(0.01,0.98,'(%s)'%(ascii_lowercase[i*3+j]),ha = 'left',va = 'top',transform = ax.transAxes)
        if i == 0:
...
# =============================================================================
# Initialise figure
fig_S5, axs_S5 = plt.subplots( ncols = 3, nrows = 4, figsize = ( 18, 16 ) )
plt.subplots_adjust( wspace = 0.25, hspace = 0.25 )
...
# values
# =============================================================================
NRMSE_values = {'LTO':{'Fagus':{'A':np.nan,'gs':np.nan,'dT':np.nan,'ci':np.nan}, 'Tilia':{'A':np.nan,'gs':np.nan,'dT':np.nan,'ci':np.nan}, 'Quercus':{'A':np.nan,'gs':np.nan,'dT':np.nan,'ci':np.nan}},
                'LT':{'Fagus':{'A':np.nan,'gs':np.nan,'dT':np.nan,'ci':np.nan}, 'Tilia':{'A':np.nan,'gs':np.nan,'dT':np.nan,'ci':np.nan}, 'Quercus':{'A':np.nan,'gs':np.nan,'dT':np.nan,'ci':np.nan}},
                'AT':{'Fagus':{'A':np.nan,'gs':np.nan,'dT':np.nan,'ci':np.nan}, 'Tilia':{'A':np.nan,'gs':np.nan,'dT':np.nan,'ci':np.nan}, 'Quercus':{'A':np.nan,'gs':np.nan,'dT':np.nan,'ci':np.nan}}}
...
for i, species in enumerate(plant_species):
    ...
    axs_5[0,i].plot(T_cuv, A, color = 'black', marker = 'o', linestyle = 'none',markerfacecolor = 'none', alpha = 1.0)
    axs_5[1,i].plot(T_cuv, gs_h2o_mol, color = 'black', marker = 'o', linestyle = 'none', markerfacecolor = 'none', alpha = 1.0 )
    axs_5[2,i].plot(T_cuv, ci, color = 'black', marker = 'o', linestyle = 'none', markerfacecolor = 'none', alpha = 1.0 )
    axs_5[3,i].plot(T_cuv, dT, color = 'black', marker = 'o', linestyle = 'none',markerfacecolor = 'none')
    ...
    axs_S5[2,i].plot(T_cuv, ci, color = 'black', marker = 'o', linestyle = 'none', markerfacecolor = 'none', alpha = 1.0 )
    axs_S5[3,i].plot(T_cuv, dT, color = 'black', marker = 'o', linestyle = 'none',markerfacecolor = 'none')
    ...
    if re_calc_loess:
        ...
        # ci
        x_fit_ci, y_fit_ci, y_err_ci = bootstrap_loess( T_cuv[idx_lpr_X].values[sort_idx_X], ci[idx_lpr_X].values[sort_idx_X], frac = 0.8, n_boot = 1000, degree = 1)
        np.save('LOESS/loess_boot_strap_ci_cuv_%s_Jones.npy'%(species), ( x_fit_ci, y_fit_ci, y_err_ci ) )
        ...
        x_fit_ci, y_fit_ci, y_err_ci    = np.load('LOESS/loess_boot_strap_ci_cuv_%s_Jones.npy'%(species))
    ...
    # =============================================================================
    # Photosynthesis
    axs_5[0,i].plot(x_fit_A, y_fit_A, color = 'black' )
    axs_5[0,i].fill_between(x_fit_A, y_fit_A - 2 * y_err_A, y_fit_A + 2 * y_err_A, color = 'black', alpha = 0.2 )
    ...
    axs_5[1,i].plot(x_fit_g, y_fit_g, color = 'black' )
    axs_5[1,i].fill_between(x_fit_g, y_fit_g - 2 * y_err_g, y_fit_g + 2 * y_err_g, color = 'black', alpha = 0.2 )
    ...
    # ci
    axs_5[2,i].plot(x_fit_ci, y_fit_ci, color = 'black' )
    axs_5[2,i].fill_between(x_fit_ci, y_fit_ci - 2 * y_err_ci, y_fit_ci + 2 * y_err_ci, color = 'black', alpha = 0.2 )
    
    axs_S5[2,i].plot(x_fit_ci, y_fit_ci, color = 'black' )
    axs_S5[2,i].fill_between(x_fit_ci, y_fit_ci - 2 * y_err_ci, y_fit_ci + 2 * y_err_ci, color = 'black', alpha = 0.2 )
    
    # Delta T vs gs
    axs_5[3,i].plot(x_fit_dT, y_fit_dT, color = 'black' )
    axs_5[3,i].fill_between(x_fit_dT, y_fit_dT - 2 * y_err_dT, y_fit_dT + 2 * y_err_dT, color = 'black', alpha = 0.2 )
    
    axs_S5[3,i].plot(x_fit_dT, y_fit_dT, color = 'black' )
    axs_S5[3,i].fill_between(x_fit_dT, y_fit_dT - 2 * y_err_dT, y_fit_dT + 2 * y_err_dT, color = 'black', alpha = 0.2 )
    ...
    axs_5[0,i].plot(T_cuv_x, A_LTO_x * 1.0e6, color = model_colors['Leaf Temperature within Optimisation (LTO)'])
    axs_5[0,i].plot(T_cuv_x, A_LT_x * 1.0e6 , color = model_colors['Leaf Temperature (LT)'])
    axs_5[0,i].plot(T_cuv_x, A_AT_x * 1.0e6 , color = model_colors['Air Temperature (AT)'])
    
    # ========== Stomatal Conductance ================   
    axs_5[1,i].plot(T_cuv_x, gs_LTO_x, color = model_colors['Leaf Temperature within Optimisation (LTO)'])
    axs_5[1,i].plot(T_cuv_x, gs_LT_x, color = model_colors['Leaf Temperature (LT)'])
    axs_5[1,i].plot(T_cuv_x, gs_AT_x, color = model_colors['Air Temperature (AT)'])
    
    # ========== Internal leaf CO2 ===================   
    axs_5[2,i].plot(T_cuv_x, ci_LTO_x, color = model_colors['Leaf Temperature within Optimisation (LTO)'])
    axs_5[2,i].plot(T_cuv_x, ci_LT_x, color = model_colors['Leaf Temperature (LT)'])
    axs_5[2,i].plot(T_cuv_x, ci_AT_x, color = model_colors['Air Temperature (AT)'])
    
    # ============== Delta T vs gs ===================
    axs_5[3,i].plot(T_leaf_LTO_x, T_leaf_LTO_x - T_cuv_x, color = model_colors['Leaf Temperature within Optimisation (LTO)'])
    axs_5[3,i].plot(T_leaf_LT_x, T_leaf_LT_x - T_cuv_x, color = model_colors['Leaf Temperature (LT)'])
    axs_5[3,i].plot(T_leaf_AT_x, T_leaf_AT_x - T_cuv_x, color = model_colors['Air Temperature (AT)'])
    ...
    # ========== Internal leaf CO2 ===================   
    axs_S5[2,i].plot(T_cuv_x, ci_LTO_x, color = model_colors['Leaf Temperature within Optimisation (LTO)'])
    axs_S5[2,i].plot(T_cuv_x, ci_LT_x_S5, color = model_colors['Leaf Temperature (LT)'])
    axs_S5[2,i].plot(T_cuv_x, ci_AT_x_S5, color = model_colors['Air Temperature (AT)'])
    
    # ============== Delta T vs gs ===================
    axs_S5[3,i].plot(T_leaf_LTO_x, T_leaf_LTO_x - T_cuv_x, color = model_colors['Leaf Temperature within Optimisation (LTO)'])
    axs_S5[3,i].plot(T_leaf_LT_x_S5, T_leaf_LT_x_S5 - T_cuv_x, color = model_colors['Leaf Temperature (LT)'])
    axs_S5[3,i].plot(T_leaf_AT_x_S5, T_leaf_AT_x_S5 - T_cuv_x, color = model_colors['Air Temperature (AT)'])
    ...
    # ci
    NRMSE_ci_LTO = mse( ci[~chi.isna()], ci_LTO[~chi.isna()]   ) ** 0.5 / ci.mean(skipna = True) * 100
    NRMSE_ci_LT  = mse( ci[~chi.isna()], ci_LT[~chi.isna()] ) ** 0.5 / ci.mean(skipna = True) * 100
    NRMSE_ci_AT  = mse( ci[~chi.isna()], ci_AT[~chi.isna()] ) ** 0.5 / ci.mean(skipna = True) * 100
    # dT
    ...
    NRMSE_values['LTO'][species]['ci'] = NRMSE_ci_LTO
    NRMSE_values['LT'][species]['ci']  = NRMSE_ci_LT
    NRMSE_values['AT'][species]['ci']  = NRMSE_ci_AT
...
fig_5.savefig('Figures/Figure_5.jpg', dpi = 300, bbox_inches = 'tight')    
...
fig_5.suptitle('Figure 5', size = 20)
